...
class DropdownWithSearch<T> extends StatelessWidget {
  ...
  final bool disabled;
  final String label;
  final String Function(T? value) selectedText;
  final String Function(T? value) dialogSelectionItemText;
  ...
  const DropdownWithSearch(
      ...
      required this.dialogSelectionItemText})
      : super(key: key);
      ...
      child: GestureDetector(
        onTap: () {
          showDialog(
              ...
              builder: (context) => SearchDialog<T>(
                  ...
                  dialogSelectionItemText: dialogSelectionItemText,
                  title: title,
        ...
        child: Container(
          ...
          child: Row(
            children: [
              Expanded(
                  child: Text(selectedText.call(selected),
                      overflow: TextOverflow.ellipsis, style: selectedItemStyle ?? const TextStyle(fontSize: 14))),
...
class SearchDialog<T> extends StatefulWidget {
  ...
  @override
  SearchDialogState<T> createState() => SearchDialogState();
}

class SearchDialogState<T> extends State<SearchDialog<T>> {
  ...
  void _onTextChanged() {
    setState(() {
      if (textController.text.isEmpty) {
        ...
        filteredList = widget.items
            .where((element) =>
                (widget.dialogSelectionItemText(element)).toLowerCase().contains(textController.text.toLowerCase()))
            .toList();
      }
    });
  }
                    ...
                    itemBuilder: (context, index) {
                      return InkWell(
                          ...
                          child: Padding(
                            ...
                            child: Text(
                              widget.dialogSelectionItemText.call(filteredList[index]),
                              style: widget.itemStyle ?? const TextStyle(fontSize: 14),
                            ),
                          ));
                    }),
